...
class UserData(BaseModel):
    """Representation of a User's data in Howblox
...
class UserAvatar(BaseModel):
    """Type definition for a Roblox user's avatar from the Howblox Info API."""
...
async def get_user_account(
    ...
    Raises:
        UserNotVerified: If raise_errors and user is not linked with Howblox.
    ...
    howblox_user = await fetch_user_data(user_id, "robloxID", "robloxAccounts")

    if guild_id:
        guild_accounts = (howblox_user.robloxAccounts or {}).get("guilds") or {}
        guild_account = guild_accounts.get(str(guild_id))

        if guild_account:
            return RobloxUser(id=guild_account)

    if howblox_user.robloxID:
        return RobloxUser(id=howblox_user.robloxID)
...
async def get_accounts(user_id: int) -> list[RobloxUser]:
    ...
    if howblox_user.robloxID:
        account_ids.add(howblox_user.robloxID)
...
async def reverse_lookup(roblox_user: RobloxUser, exclude_user_id: int | None = None) -> list[int]:
    ...
    cursor = mongo.howblox["users"].find(
