...
from howblox_lib.models import users, guilds
...
async def fetch_item[T](domain: str, constructor: Type[T], item_id: str, *aspects) -> T:
    ...
    if not item:
        item = await mongo.howblox[domain].find_one({"_id": item_id}, {x: True for x in aspects}) or {
            "_id": item_id
        }
...
async def update_item(domain: str, item_id: str, **aspects) -> None:
    ...
    # update database
    await mongo.howblox[domain].update_one(
